...
def migrate_block_v1_to_v2(block: dict, page_width: int, page_height: int) -> Tuple[dict, List[str]]:
    """
    ...
    if "coords_norm" not in migrated:
        ...
        if page_width > 0 and page_height > 0:
            ...
            warnings.append(f"Блок {block.get('id', '?')}: coords_norm установлены по умолчанию (нет размеров страницы)")
...
def migrate_annotation_data(data: dict) -> Tuple[dict, MigrationResult]:
    ...
    for page in data.get("pages", []):
        ...
        for block in page.get("blocks", []):
            ...
            if missing:
                all_warnings.append(
                    f"Блок пропущен - отсутствуют поля: {missing}"
                )
                continue
