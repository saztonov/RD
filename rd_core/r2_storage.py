    ...
    def __init__(
        ...
        try:
            ...
            client_config = Config(
                retries={"max_attempts": 10, "mode": "adaptive"},  # Адаптивные retry с backoff
                connect_timeout=30,
                read_timeout=120,
...
def upload_ocr_to_r2(output_dir: str, project_name: Optional[str] = None) -> bool:
    ...
    try:
        ...
        logger.error("   Убедитесь что в .env указаны: R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY")
        return False
    except Exception as e:
