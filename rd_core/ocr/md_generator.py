...
def _collect_image_text_links_from_pages(pages: List) -> Dict[str, str]:
    """
...
def _collect_image_text_links_from_result(pages: List[Dict]) -> Dict[str, str]:
    """
...
def _get_text_block_content(
    ...
    if is_dict:
        ...
        if ocr_html:
            return _html_to_markdown(ocr_html)
        elif ocr_text:
            return _process_ocr_content(ocr_text)
    else:
