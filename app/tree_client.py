...
from app.tree_models import (
    NodeType, NodeStatus, FileType, NodeFile, TreeNode, StageType, SectionType,
    ALLOWED_CHILDREN
)

# Реэкспорт для обратной совместимости
__all__ = [
    'NodeType', 'NodeStatus', 'FileType', 'NodeFile', 'TreeNode', 
    'StageType', 'SectionType', 'TreeClient', 'ALLOWED_CHILDREN'
]
...
class TreeClient:
    ...
    def create_image_category(
        ...
        if is_default:
            self._request("patch", "/image_categories?is_default=eq.true", json={"is_default": False})
        
        resp = self._request("post", "/image_categories", json=payload)
        return resp.json()[0]
    
    def update_image_category(self, category_id: str, **fields) -> Optional[Dict[str, Any]]:
        """Обновить категорию изображений"""
        update_data = {}
        for key in ["name", "code", "description", "system_prompt", "user_prompt", "is_default", "sort_order"]:
            if key in fields:
        ...
        if update_data.get("is_default"):
            self._request("patch", f"/image_categories?is_default=eq.true&id=neq.{category_id}", json={"is_default": False})
        
        resp = self._request("patch", f"/image_categories?id=eq.{category_id}", json=update_data)
        data = resp.json()
        return data[0] if data else None
