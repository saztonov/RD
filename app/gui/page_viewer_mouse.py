        ...
        if event.button() == Qt.LeftButton:
            ...
            if event.modifiers() & Qt.ControlModifier:
                if clicked_block is not None:
                    ...
                    if (
                        self.selected_block_idx is not None
                        and self.selected_block_idx not in self.selected_block_indices
                    ):
                ...
                if 0 <= self.selected_block_idx < len(self.current_blocks):
                    ...
                    if not self.read_only:
                        if (
                            ...
                            and block.polygon_points
                        ):
                            vertex_idx = self._get_polygon_vertex_handle(
                                scene_pos, block.polygon_points
                            )
                            ...
                            edge_idx = self._get_polygon_edge_handle(
                                scene_pos, block.polygon_points
                            )
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.right_button_pressed and not self.selecting and self.start_point:
            ...
            if distance > 5:
                self.selecting = True
                self.rubber_band_item = QGraphicsRectItem(
                    QRectF(self.start_point, clamped_pos)
                )
        ...
        if self.selecting and self.start_point and self.rubber_band_item:
            ...
            and self.selected_block_idx is not None
        ):
            self._update_polygon_vertex(
                self.selected_block_idx, self.dragging_polygon_vertex, clamped_pos
            )

        elif (
            self.dragging_polygon_edge is not None
            and self.selected_block_idx is not None
        ):
            delta = clamped_pos - self.move_start_pos
            self._update_polygon_edge(
                self.selected_block_idx, self.dragging_polygon_edge, delta
            )
            ...
            if (
                self.selected_block_idx is not None
                and 0 <= self.selected_block_idx < len(self.current_blocks)
            ):
                block = self.current_blocks[self.selected_block_idx]
                x1, y1, x2, y2 = block.coords_px
                block_rect = QRectF(x1, y1, x2 - x1, y2 - y1)
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            if self.drawing:
                ...
                self.moving_block
                or self.resizing_block
                or self.dragging_polygon_vertex is not None
                or self.dragging_polygon_edge is not None
            ):
                if (
                    self.selected_block_idx is not None
                    and 0 <= self.selected_block_idx < len(self.current_blocks)
