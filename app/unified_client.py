...
class UnifiedClient:
    ...
    def update_node(self, node_id: str, **fields) -> Optional[TreeNode]:
        ...
        try:
            resp = self._request(
                "patch", f"/api/tree/nodes/{node_id}", json=update_data
            )
            return TreeNode.from_dict(resp.json())
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 404:
    ...
    def upload_file(
        self, local_path: str, r2_key: str, content_type: Optional[str] = None
    ) -> bool:
        """Загрузить файл в R2"""
        try:
            local_file = Path(local_path)
    ...
    def upload_text(
        self, content: str, r2_key: str, content_type: Optional[str] = None
    ) -> bool:
