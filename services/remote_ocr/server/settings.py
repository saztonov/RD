...
def _get_setting(db_settings: Optional[dict], key: str, env_key: str, default, cast_fn=None):
...
class Settings:
    ...
    data_dir: str = field(default_factory=lambda: os.getenv("REMOTE_OCR_DATA_DIR", "/data"))
    api_key: str = field(default_factory=lambda: os.getenv("REMOTE_OCR_API_KEY", ""))
    openrouter_api_key: str = field(default_factory=lambda: os.getenv("OPENROUTER_API_KEY", ""))
    datalab_api_key: str = field(default_factory=lambda: os.getenv("DATALAB_API_KEY", ""))
    ...
    max_concurrent_jobs: int = field(
        default_factory=lambda: _get_setting(_db_settings, "max_concurrent_jobs", "MAX_CONCURRENT_JOBS", 4, int)
    )
    worker_prefetch: int = field(
        default_factory=lambda: _get_setting(_db_settings, "worker_prefetch", "WORKER_PREFETCH", 1, int)
    )
    ...
    task_retry_delay: int = field(
        default_factory=lambda: _get_setting(_db_settings, "task_retry_delay", "TASK_RETRY_DELAY", 60, int)
    )
    
    # ===== OCR THREADING =====
    max_global_ocr_requests: int = field(
        default_factory=lambda: _get_setting(_db_settings, "max_global_ocr_requests", "MAX_GLOBAL_OCR_REQUESTS", 8, int)
    )
    ocr_threads_per_job: int = field(
        default_factory=lambda: _get_setting(_db_settings, "ocr_threads_per_job", "OCR_THREADS_PER_JOB", 2, int)
    )
    ocr_request_timeout: int = field(
        default_factory=lambda: _get_setting(_db_settings, "ocr_request_timeout", "OCR_REQUEST_TIMEOUT", 120, int)
    )
    
    # ===== DATALAB API =====
    datalab_max_rpm: int = field(
        default_factory=lambda: _get_setting(_db_settings, "datalab_max_rpm", "DATALAB_MAX_RPM", 180, int)
    )
    ...
    max_ocr_batch_size: int = field(
        default_factory=lambda: _get_setting(_db_settings, "max_ocr_batch_size", "MAX_OCR_BATCH_SIZE", 5, int)
    )
    pdf_render_dpi: int = field(
        default_factory=lambda: _get_setting(_db_settings, "pdf_render_dpi", "PDF_RENDER_DPI", 300, int)
    )
    max_strip_height: int = field(
        default_factory=lambda: _get_setting(_db_settings, "max_strip_height", "MAX_STRIP_HEIGHT", 9000, int)
    )
    
    # ===== ОЧЕРЕДЬ =====
    poll_interval: float = field(
        default_factory=lambda: _get_setting(_db_settings, "poll_interval", "POLL_INTERVAL", 10.0, float)
    )
    poll_max_interval: float = field(
        default_factory=lambda: _get_setting(_db_settings, "poll_max_interval", "POLL_MAX_INTERVAL", 60.0, float)
    )
    max_queue_size: int = field(
        default_factory=lambda: _get_setting(_db_settings, "max_queue_size", "MAX_QUEUE_SIZE", 100, int)
    )
    default_task_priority: int = field(
        default_factory=lambda: _get_setting(_db_settings, "default_task_priority", "DEFAULT_TASK_PRIORITY", 5, int)
    )
