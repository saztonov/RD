...
router = APIRouter(prefix="/api/storage", tags=["storage"], dependencies=[Depends(verify_api_key)])
...
def delete_by_prefix_endpoint(prefix: str):
    ...
    try:
        ...
        return {"deleted_count": len(deleted), "error_count": len(errors), "deleted": deleted}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
