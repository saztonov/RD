...
def pass1_prepare_crops(
    ...
    block_crop_paths: Dict[
        str, List[Tuple[str, int, int]]
    ] = {}  # block_id -> [(path, part_idx, total_parts)]
                ...
                try:
                    ...
                    if save_image_crops_as_pdf and block.block_type == BlockType.IMAGE:
                        ...
                        if result:
                            ...
                            # Обновляем первую часть entry с путём к PDF
...
def _group_and_merge_strips(
    ...
    ] = []  # (block_id, crop_path, part_idx, total_parts)
...
def pass2_ocr_from_manifest(
    ...
    def _process_image(entry: CropManifestEntry):
        ...
        try:
            # Извлекаем pdfplumber текст (не требует изображения)
            ...
            # Проверяем возможность использования native PDF
            use_native_pdf = (
                entry.pdf_crop_path
                and entry.total_parts == 1  # Только одночастные блоки
